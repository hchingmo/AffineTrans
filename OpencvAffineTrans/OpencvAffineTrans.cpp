// OpencvAffineTrans.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
   ...
   */
    ...
    */
    ...
    // 2. improve the holes on image due to rotation by using linear interpolation
    /*
    for (int i = 1; i < row - 1; i++)
    {
        dataTrans = imageBTrans.ptr(i);
        ...
        for (int j = 1; j < col - 1; j++)
        {
            int row_u_inter = dataTrans[j - col - 1] + (dataTrans[j - col + 1] - dataTrans[j - col - 1]) / 2;
            int row_l_inter = dataTrans[j + col - 1] + (dataTrans[j + col + 1] - dataTrans[j + col - 1]) / 2;
            dataTransOpt[j] = row_u_inter + (row_l_inter - row_u_inter) / 2;
        }
    }
    */
...
/*
   ...
   // imshow("imageBTrans", imageBTrans);
